...
public class WebSocketConfig implements WebSocketMessageBrokerConfigurer {
	...
	public void configureMessageBroker(MessageBrokerRegistry config) {
		config.enableSimpleBroker("/topic"); //서버에서 클라이언트측으로 메세지 보낼 때 붙여주는 prefix
		config.setApplicationDestinationPrefixes("/app"); //클라이언트가 메세지를 서버쪽으로 전송할때 url정보 앞에 붙여주는 prefix
	}
